...
def get_tp_fp_fn_tn(net_output, gt, axes=None, mask=None, square=False):
    ...
    #print(shp_x, shp_y) # torch.Size([8, 9, 64, 128, 128]) torch.Size([8, 1, 64, 128, 128])
        ...
        if all([i == j for i, j in zip(net_output.shape, gt.shape)]):
            ...
            if net_output.device.type == "cuda":
                try:
                    ...
                    print(net_output.device.index) # 0?
                    print(e)
                    raise Exception("buig..")
            y_onehot.scatter_(1, gt, 1)
            #print(y_onehot.shape) // torch.Size([8, 9, 64, 128, 128])
    ...
    if square:
        ...
        fn = fn ** 2
        tn = tn ** 2
    #print(tp.shape) # torch.Size([8, 9, 64, 128, 128])
    #print(fp.shape) # torch.Size([8, 9, 64, 128, 128])
    #print(fn.shape) # torch.Size([8, 9, 64, 128, 128])
    if len(axes) > 0:
