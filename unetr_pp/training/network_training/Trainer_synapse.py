...
class Trainer_synapse(NetworkTrainer_synapse):
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
        ...
        self.loss = DC_and_CE_and_HD_loss({'batch_dice': self.batch_dice, 'smooth': 1e-5, 'do_bg': False}, {}, focal=True)
        #self.loss = AsymmetricUnifiedFocalLoss()
        #self.loss = DC_and_FCE_loss(({'batch_dice': self.batch_dice, 'smooth': 1e-5, 'do_bg': False}, {}))
        ...
        self.lr_scheduler_eps = 1e-3 # 1e-3 -> 1e-5
    ...
    def validate(self, do_mirroring: bool = True, use_sliding_window: bool = True, step_size: float = 0.5,
        ...
        self.network.train(current_mode)
    '''
    def remove_all_but_the_largest_connected_component(self, image: np.ndarray, for_which_classes: list, volume_per_voxel: float,
    ...
    def post_processing(self, data, whole=False):

        processed, largest_removed, kept_size = self.remove_all_but_the_largest_connected_component(
            image=data,
            for_which_classes=[1],
            volume_per_voxel=3,
    ...
    def file_inference(self, do_mirroring: bool = True, use_sliding_window: bool = True, step_size: float = 0.5,
                 save_softmax: bool = True, use_gaussian: bool = True, overwrite: bool = True,
                 validation_folder_name: str = 'validation_raw', debug: bool = False, all_in_gpu: bool = False,
                 segmentation_export_kwargs: dict = None, run_postprocessing_on_folds: bool = True, test_path = '', output_folder='',
                 post=True, spacing_origin=True, type='nii'):
        current_mode = self.network.training
        self.network.eval()
        ...
        #####
        # input numpy: consist of label and CT
        # TODO: changing function consturction
        ####
        import os
        test_file = os.listdir(test_path)
        npy_list = [file for file in test_file if 'npy' in file]
        validation_folder = os.listdir(output_folder)
        for k in npy_list:
            #properties = load_pickle(self.dataset[k]['properties_file'])
            fname = k # path
            nii_name = fname
            ...
            #        (save_softmax and not isfile(join(output_folder, fname + ".npz"))):
                ...
                #print(data[1])
                print(k, data.shape)
                #print("Warning: Github processing...!!!")
                #data[-1] = np.where(data[-1] == 1, 6, 0) # orginal class 6 is whole liver

                softmax_pred = self.predict_preprocessed_data_return_seg_and_softmax(data[:-1], # TODO: 임시로 추가함.
                                                                                     do_mirroring=do_mirroring,
                                                                                     mirror_axes=mirror_axes,
                                                                                     use_sliding_window=use_sliding_window,
                                                                                     step_size=step_size,
                ...
                # 1. argmax
                #print(softmax_pred)
                softmax_pred = softmax_pred.argmax(0, keepdims=1) # axis=0 기준으로 binary 
                softmax_pred = np.array(softmax_pred, dtype='int')
                softmax_pred = softmax_pred.transpose(0,3,2,1)
                #print(softmax_pred.shape) # (1, 245, 471, 471)

                # 2-1. save npy and nii.gz format
                #np.save(output_folder+"/"+fname, softmax_pred) # validataion_raw에 저장 # 2 ckasses
                print('############ ' + type + ' saving... ############')
                if type == 'nii':
                    self.nii_saving(test_path, k, '.nii.gz', fname, softmax_pred, output_folder, post, spacing_origin)
                elif type == 'mha':
                    self.mha_saving(test_path, k, '.mha', fname, softmax_pred, output_folder, post, spacing_origin)
                else:
                    raise Exception('No support file extension')
                
    def mha_saving(self, test_path, k, extension, fname, softmax_pred, output_folder, post, spacing_origin):
        from medpy.io import save
        #### 1차적인 대안 -> 추후 develop #####################################
        import SimpleITK as sitk
        mha_file = sitk.ReadImage(os.path.join(test_path, k[:-4]+extension))
        original_spacing = mha_file.GetSpacing()

        extension = '.nii.gz'
        sitk.WriteImage(mha_file, os.path.join(test_path, k[:-4]+extension))
        real = nib.load(os.path.join(test_path, k[:-4]+extension))
        ######################################################################

        mha_name = 'PRED_' + fname[:-4] + extension
        nifti_file = nib.Nifti1Image(softmax_pred[0], real.affine, real.header) # just foreground
        nifti_file.to_filename(os.path.join(output_folder, mha_name))  # Save as NiBabel file
        
        if post:
            ############ Post processing #######
            print('########## Post processing ############')
            pred_array = tio.ScalarImage(os.path.join(output_folder, mha_name))
            pred_array = np.array(pred_array)
            print(pred_array.shape)
            pred_array = pred_array[0]

            post_pred = self.post_processing(pred_array)
            post_pred = post_pred[np.newaxis,]

            # Resave
            mha_name = 'POST_PRED_' + fname[:-4] + extension
            nifti_file = nib.Nifti1Image(post_pred[0], real.affine, real.header)
            nifti_file.to_filename(os.path.join(output_folder, mha_name))

            if spacing_origin:
                print('########## Recover processing ############')
                #from nibabel.affines import voxel_sizes
                #real = nib.load(os.path.join(test_path,  k[:-4]+extension))
                #spacing = voxel_sizes(real.affine)
                
                # Recover spacing
                resample = tio.Resample(tuple(original_spacing))
                #pred_array = tio.ScalarImage(os.path.join(output_folder, nii_name))
                #pred_array = np.array(pred_array.data)
                resampled = resample(post_pred) # pred_array
                pred_array = resampled.data
                pred_array = np.array(pred_array, dtype='int')

                ### resize 옵션 나중에 추가 ###

                if pred_array.shape[1:] != real.shape:
                    print(pred_array.shape, '->', real.shape, '변환')
                    resize = tio.Resize(real.shape)
                    pred_array = resize(pred_array)
                ##############################

                # Resave
                mha_name = 'Spacing_POST_PRED_' + fname[:-4] + extension
                nifti_file = nib.Nifti1Image(pred_array[0], real.affine, real.header)
                nifti_file.to_filename(os.path.join(output_folder, mha_name))  
        print('######### All process - Done #########')
                
    def nii_saving(self, test_path, k, extension, fname, softmax_pred, output_folder, post, spacing_origin):
        real = nib.load(os.path.join(test_path, k[:-4]+extension))
        nii_name = 'PRED_' + fname[:-4] + extension
        nifti_file = nib.Nifti1Image(softmax_pred[0], real.affine, real.header) # just foreground
        nifti_file.to_filename(os.path.join(output_folder, nii_name))  # Save as NiBabel file
        ...
        if post:
            ...
            if spacing_origin:
                ...
                # Recover spacing
                resample = tio.Resample(tuple(spacing))
                #pred_array = tio.ScalarImage(os.path.join(output_folder, nii_name))
                #pred_array = np.array(pred_array.data)
                resampled = resample(post_pred) # pred_array
                pred_array = resampled.data
                ...
                # Resave
                nii_name = 'Spacing_POST_PRED_' + fname[:-4] + extension
                nifti_file = nib.Nifti1Image(pred_array[0], real.affine, real.header)
                nifti_file.to_filename(os.path.join(output_folder, nii_name))

            else:
                # Resave
                nii_name = 'POST_PRED_' + fname[:-4] + extension
                nifti_file = nib.Nifti1Image(post_pred[0], real.affine, real.header)
                nifti_file.to_filename(os.path.join(output_folder, nii_name))
        print('######### All process - Done #########')
    ...
    def validate(self, do_mirroring: bool = True, use_sliding_window: bool = True, step_size: float = 0.5,
        ...
        ####
        import os
        test_path = '../DATASET_Synapse/unetr_pp_raw/unetr_pp_raw_data/Task02_Synapse/Task002_Synapse/unetr_pp_Data_plans_v2.1_stage1/'+data_name+'/'
        test_file = os.listdir(test_path)
        npy_list = [file for file in test_file if 'npy' in file]
        validation_folder = os.listdir(output_folder)
        for k in npy_list:
            ...
            #        (save_softmax and not isfile(join(output_folder, fname + ".npz"))):
                ...
                import nibabel as nib
                nii_name = fname[:-4] + '.nii'
                nifti_file = nib.Nifti1Image(softmax_pred[0], np.eye(4)) # just foreground
                nifti_file.header.get_xyzt_units()
                nifti_file.to_filename(os.path.join(output_folder, nii_name))  # Save as NiBabel file

                # 2-2. save original label (check seg_gt folder)
                # (skip)
                ##################################

            self.gt_niftis_folder = '../DATASET_Synapse/unetr_pp_raw/unetr_pp_raw_data/Task02_Synapse/Task002_Synapse/seg_gt/'+data_name+'/'
            
            # LiTS: nii // synapse: nii
            #pred_gt_tuples.append([join(output_folder, fname[:-4] + ".nii"),
            #                       join(self.gt_niftis_folder, fname[:-4].replace('volume', 'segmentation') + ".nii")])
            ### MSD version
            pred_gt_tuples.append([join(output_folder, fname[:-4] + ".nii"),
                                   join(self.gt_niftis_folder, fname[:-4] + ".nii.gz")])
        ...
        if len(inference_times) != 0:
            print(f"Time per Image: {sum(inference_times) / len(inference_times)} sec.")
        #_ = [i.get() for i in results) # save하는 코드 위로
        self.print_to_log_file("finished prediction")
